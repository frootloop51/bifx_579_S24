...
library(recount)
## This is how to download the R summarized experiment based on gene counts using the SRA accession number for the experiment
...
## extract count data from rse
immunoGeneCounts <- assays(rse_gene)$counts
## save counts as a .csv
write.csv(immunoGeneCounts, file="~/Dropbox/WomenHackathon/immunoGeneCounts.csv")

## extract sample metadata from rse
immunoColData <- colData(rse_gene)
write.csv(immunoColData, file="~/Dropbox/WomenHackathon/immunoColData.csv")

#alternatively, you could enter another workflow at this point such as differential expression using DESeq2
