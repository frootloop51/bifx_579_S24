...
f2 <- function(df){
  (df[which(df$samples_count >= length(unlistsplitspinalc) *.60), ])
}
#Apply function to list of dataframes:
dfList2 <- lapply( X=spinalc, FUN=f2 ) 
#Bind the lists into one dataframe:
allDF2 <-do.call(rbind,dfList2) 
#Write the dataframe to a .csv: (NOTE: ADD ROWNAMES)
write.csv(allDF2, "~/Documents/snaptronwork/snapout_analysis/spinalc_sixtypercent.csv")
#Check output:
#spinalc_sixtypercent <- read.csv("~/Documents/snaptronwork/snapout_analysis/spinalc_sixtypercent.csv", header = TRUE, sep = ",")
